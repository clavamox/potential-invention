...
public class BGAEmptyView extends FrameLayout {
    ...
    protected void onFinishInflate() {
        ...
        if (getChildCount() < 0 || getChildCount() > 2) {
            throw new IllegalStateException("BGAEmptyView只能有一个或两个子控件");
        }
        initView();
        setListener();
        showContentView();
    ...
    private void initView() {
        if (getChildCount() == 1) {
            ...
            this.mEmptyView = getViewById(R.id.ll_bga_adapter_empty_view_root);
        } else {
            this.mEmptyView = getChildAt(0);
            this.mContentView = getChildAt(1);
        }
        this.mMsgTv = (TextView) getViewById(R.id.tv_bga_adapter_empty_view_msg);
        this.mIconIv = (ImageView) getViewById(R.id.iv_bga_adapter_empty_view_icon);
    }
